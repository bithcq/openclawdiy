...
git apply --check ~/openclawdiy/patches/0001-native-web-search.patch
```
...
# 2. 查看旧补丁改了哪些文件、改了什么
cat ~/openclawdiy/patches/0001-native-web-search.patch

# 3. 在官方代码上手工应用同等逻辑的改动
#    （编辑对应文件，保持改动意图不变，适配新的上下文）

# 4. 生成新补丁（只包含被改动的文件）
git diff src/agents/pi-embedded-runner/compact.ts \
         src/agents/pi-embedded-runner/run/attempt.ts \
  > ~/openclawdiy/patches/0001-native-web-search.patch
...
| 补丁 | 改动文件 | 改动意图 |
|------|---------|---------|
| `0001-native-web-search.patch` | `compact.ts`, `run/attempt.ts` | 让 baseUrl 模式自动切换为模型原生 web_search |
| `0002-plugin-route-registry-alignment.patch` | `server-channels.ts`, `server-channels.test.ts`, `server.impl.ts` | 防止插件动态注册 HTTP 路由时 registry 漂移 |
...
- **推动上游合并**：将 native-web-search 和 plugin-route-registry 的改动提交为官方 PR，合并后可删除对应 patch
- **监控官方变更**：关注 `src/agents/pi-embedded-runner/` 和 `src/gateway/server-channels.ts` 的 commit，提前发现冲突
